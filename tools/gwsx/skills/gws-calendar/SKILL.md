...
metadata:
  ...
  openclaw:
    ...
    requires:
      bins:
        - gwsx
    cliHelp: "gwsx <account> calendar --help"
---
...
```bash
gwsx <account> calendar <resource> <method> [flags]
```
...
gwsx <account> calendar --help

# Inspect a method's required params, types, and defaults
gwsx <account> schema calendar.<resource>.<method>
```

Use `gwsx <account> schema` output to build your `--params` and `--json` flags.
